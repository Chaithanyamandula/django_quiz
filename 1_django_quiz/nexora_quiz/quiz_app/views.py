...
from django.db import IntegrityError # Import for handling unique constraint errors
...
def start_quiz(request):
    if request.method == 'POST':
        ...
        if not all([contestant_name, contestant_email, selected_language]):
            messages.error(request, "Please fill in all fields and select a language.")
        ...
        try:
            ...
            request.session['contestant_name'] = contestant.name
            request.session['contestant_email'] = contestant.email
            ...
            if len(all_questions_for_lang) < 5:
                messages.error(request, f"Not enough questions available for {selected_language}. Please choose another.")
                return redirect(reverse('index'))
            ...
            return redirect(reverse('index'))
    return redirect(reverse('index')) # If GET request to start_quiz, redirect to index
    ...
    if current_question_index < len(quiz_questions):
        ...
        if contestant_id:
            try:
                ...
                print("Contestant not found for storing results.")
            except Exception as e:
        ...
        send_quiz_result_email(request.session['contestant_email'], request.session['contestant_name'], language, score, total_questions)
...
def result(request):
    ...
    session_keys_to_clear = [
        'quiz_questions', 'current_question_index', 'score',
        'contestant_id', 'contestant_name', 'contestant_email', 'selected_language'
    ]
